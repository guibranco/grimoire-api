    ...
    IEncryptionService encryption) : ControllerBase
{
    ...
    public async Task<IActionResult> List(string slug, CancellationToken ct)
    {
        ...
        if (app is null) return NotFound(Problem("Application not found.", statusCode: 404));

        var secrets = await secretRepo.GetByApplicationAsync(app.Id, ct);
        return Ok(secrets.Select(s => new SecretResponse(s.Id, s.Name, s.Description, s.CreatedAt, s.UpdatedAt)));
    }

    [HttpPost]
    public async Task<IActionResult> Create(string slug, [FromBody] CreateSecretRequest req, CancellationToken ct)
    {
        var app = await appRepo.GetBySlugAsync(slug, ct);
        if (app is null) return NotFound(Problem("Application not found.", statusCode: 404));

        var existing = await secretRepo.GetByNameAsync(app.Id, req.Name, ct);
        if (existing is not null) return Conflict(Problem("Secret already exists.", statusCode: 409));
        ...
        return CreatedAtAction(nameof(GetByName), new { slug, name = secret.Name },
            new CreateSecretResponse(secret.Id, secret.Name, secret.Description, secret.CreatedAt, required));
    }
    ...
    public async Task<IActionResult> GetByName(string slug, string name, CancellationToken ct)
    {
        ...
        if (secret is null) return NotFound(Problem("Secret not found.", statusCode: 404));

        return Ok(new SecretResponse(secret.Id, secret.Name, secret.Description, secret.CreatedAt, secret.UpdatedAt));
    }

    [HttpPost("{name}/values")]
    public async Task<IActionResult> SetValues(string slug, string name, [FromBody] List<SetSecretValueRequest> req, CancellationToken ct)
    {
        var app = await appRepo.GetBySlugAsync(slug, ct);
        if (app is null) return NotFound(Problem("Application not found.", statusCode: 404));
        ...
        foreach (var item in req)
        {
            ...
            if (env is null) return NotFound(Problem($"Environment '{item.EnvironmentSlug}' not found.", statusCode: 404));
    ...
    public async Task<IActionResult> GetVersions(string slug, string name, string environmentSlug, CancellationToken ct)
    {
        var app = await appRepo.GetBySlugAsync(slug, ct);
        if (app is null) return NotFound(Problem("Application not found.", statusCode: 404));
        ...
        return Ok(versions.Select(v => new SecretVersionMetadata(v.Id, v.Version, v.IsEnabled, v.ExpiresAt, v.NotBefore, v.CreatedAt)));
    }
