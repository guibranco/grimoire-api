    ...
    IEnvironmentRepository envRepo) : ControllerBase
{
    ...
    public async Task<IActionResult> List(string slug, CancellationToken ct)
    {
        ...
        if (app is null) return NotFound(NotFoundProblem("Application not found."));
    ...
    [HttpPost]
    public async Task<IActionResult> Create(string slug, [FromBody] CreateEnvironmentRequest req, CancellationToken ct)
    {
        var app = await appRepo.GetBySlugAsync(slug, ct);
        if (app is null) return NotFound(NotFoundProblem("Application not found."));
        ...
        if (existing is not null) return Conflict(new ProblemDetails { Title = "Conflict", Detail = "Environment already exists.", Status = 409 });
        ...
        return CreatedAtAction(nameof(List), new { slug }, new EnvironmentResponse(env.Id, env.Name, env.Slug, env.CreatedAt));
    }
    ...
    public async Task<IActionResult> Delete(string slug, string envSlug, CancellationToken ct)
    {
        ...
        if (env is null) return NotFound(NotFoundProblem("Environment not found."));
