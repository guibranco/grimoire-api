...
{
    [HttpGet("{name}")]
    public async Task<IActionResult> GetSecret(string name, [FromQuery] string environment, CancellationToken ct)
    {
        ...
        if (version is null) return NotFound(Problem("No active secret version found.", statusCode: 404));

        var plainValue = encryption.Decrypt(version.EncryptedValue);

        return Ok(new KeyVaultSecretResponse(
            secret.Name,
            plainValue,
