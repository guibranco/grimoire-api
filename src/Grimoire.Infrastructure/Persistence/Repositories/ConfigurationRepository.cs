...
{
    public Task<List<ConfigurationEntry>> GetByApplicationAsync(
        Guid applicationId,
        CancellationToken ct = default
    ) =>
        db
            .ConfigurationEntries.Include(c => c.Environment)
            .Where(c => c.ApplicationId == applicationId)
            .ToListAsync(ct);

    public Task<List<ConfigurationEntry>> GetByEnvironmentAsync(
        Guid applicationId,
        Guid environmentId,
        CancellationToken ct = default
    ) =>
        db
            .ConfigurationEntries.Where(c =>
                c.ApplicationId == applicationId && c.EnvironmentId == environmentId
            )
            .ToListAsync(ct);

    public Task<ConfigurationEntry?> GetByKeyAsync(
        Guid applicationId,
        Guid environmentId,
        string key,
        CancellationToken ct = default
    ) =>
        db.ConfigurationEntries.FirstOrDefaultAsync(
            c =>
