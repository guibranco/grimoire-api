...
public static class TestHelpers
{
    ...
    {
        name ??= UniqueName("app");
        var response = await mgmtClient.PostAsJsonAsync("/api/management/applications",
    ...
    public static async Task<string> CreateEnvironmentAsync(
        HttpClient mgmtClient, string appSlug, string envName = "staging")
    {
    ...
    public static async Task CreateSecretWithValueAsync(
        HttpClient mgmtClient, string appSlug, string secretName, string envSlug, string value)
    {
        ...
        var setValue = await mgmtClient.PostAsJsonAsync(
            ...
            new[] { new { environmentSlug = envSlug, value, isEnabled = true } });
